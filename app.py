...
# ── Live Analyzer — shared function (used in both tab states) ─────────────────
...
# ── Upload tab content — shared function ──────────────────────────────────────
...
# ══ TABS — Branch based on whether data exists ════════════════════════════════
if df.empty:
    tab0, tab4 = st.tabs([
        "⬆  Upload & Process",
        "Live Analyzer"
    ...
    # ── Re-load after potential processing ────────────────────────────────────
    df = load_data(db_path)
    if not df.empty:
        total   = len(df)
